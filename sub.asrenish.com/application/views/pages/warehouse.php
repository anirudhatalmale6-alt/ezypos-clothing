...
<script>
    ...
    function loadStoreItems() {
        $.ajax({
            url: "<?= base_url('StoreItems/fetch_itemwise_current_qty_with_warehouse') ?>",
            type: "GET",
            dataType: "json",
            success: function(data) {
                ...
                $.each(data, function(index, item) {
                    let row =  `<tr>
                        ...
                        <td>${item.total_current_quantity}</td>
                    </tr>`;
...
$(document).ready(function() {
    function filterTable() {
        ...
        $('#table2 tbody tr').each(function() {
            let warehouseName = $(this).find('td:nth-child(3)').text().trim();
