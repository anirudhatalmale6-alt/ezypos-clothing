...
<script>
    ...
    $(function() {
        ...
        function getSupPayments(){ //payment by supplier id
            ...
            $.ajax({
                ...
                success:function(data){
                    ...
                    sup_starting_balance=data[2];
                    if(OutstandingSup==false && sup_starting_balance==0){OutstandingSup=0;}
                    //OutstandingSup=parseFloat(OutstandingSup)+parseFloat(sup_starting_balance);
                    OutstandingSup=parseFloat(noNaN(sup_starting_balance));
                    var i;
                    ...
                    cheqlog='';
                         ...
                         new_payment_total_w=parseFloat(company_cash_val_w)+parseFloat(customer_cheque_val_w)+parseFloat(company_cheque_val_w);
                         var new_due_payment= OutstandingSup_val - new_payment_total_w;
                         new_due_payment=new_due_payment.toFixed(2);
                        $('#tbodyID').html(rows);
                        var outstanding='<tr>'+                                            
                        ...
                        var outstanding_calc= '<div class="form-group row">'+
                                    ...
                                    '<label class="col-form-label" style="margin-left: 5px; clear: both;float:left;" id="grandtotalLbl">'+new_due_payment+'</label>'+
        ...
        company_cheque_val_w = 0;
        $("#amount").keyup(function(e){
        company_cheque_val_w =Number($("#amount").val()).toFixed(2);
        calculate_new_outsatnding();
        });
        ...
        function calculate_new_outsatnding(){
          var due_payment_val_w = $("#tot_sup_out").html();  
          new_payment_total_w=parseFloat(company_cash_val_w)+parseFloat(customer_cheque_val_w)+parseFloat(company_cheque_val_w);
...
//        $("#cashvalue").keyup(function(e){
//         var due_payment_val_w = $("#tot_sup_out").html();
//         var customer_cheque_val_w = $("#pCheques").html();
//         var data_amount=0;
//         if(customer_cheque_val_w!='undefined' && customer_cheque_val_w != null){
//           alert("ok")  ;
//           //var cus_che_select_option = document.getElementById('pCheques');
//          // data_amount = cus_che_select_option.getAttribute('data-amount');
//           data_amount = documrnt.getAttribute('data-amount');
//           alert(data_amount);
//         }
//         
...
//       
//         if(cashvalue_w==""){
//            $("#grandtotalLbl").html(due_payment_val_w);
//         }else{
//
...
//get outstanding by GRN ID
        function getGrnByID(){
            ...
            $.ajax({
                ...
                success:function(data){ //pymntlog_amount,pymntlog_date 
                        ...
                        var supplier_id_w = parseFloat(data[5]);
                        if((isNaN(chqtotal))==true){chqtotal=0;}
                        //ttlOutstanding=parseFloat(data[4]); gives sum of all pending grn outstanding valus
                        ttlOutstanding=parseFloat(data[6]);
                        ttlOutstanding_w=ttlOutstanding.toFixed(2);
                        
                        var totalPaid=(paidOfCash+chqtotal).toFixed(2);

                        
                         var OutstandingGrn_val=Number(grnOutstanding).toFixed(2);
                         new_payment_total_w=parseFloat(company_cash_val_w)+parseFloat(customer_cheque_val_w)+parseFloat(company_cheque_val_w);
                         var new_due_payment= OutstandingGrn_val - new_payment_total_w;
                         new_due_payment=new_due_payment.toFixed(2);
                        ...
                        var grnInfo='<tr>'+                                            
                                        ...
                                        '<tr >'+                                            
                                            ...
                                            '<td style="text-align:right" id="tot_sup_out">'+grnOutstanding+'</td>'+
                                        '</tr>'+
                                        '<tr >'+                                            
                                            '<td>Over all outstanding</td>'+
                                            '<td style="text-align:right">'+ttlOutstanding_w+'</td>'+
                                        '</tr>';
                            ...
                            var chequesInHand =
                                '<div>'+
                                    ...
                                    '<div class="form-group row">'+
                                        ...
                                        '</div>'+
                                        '<a href="javascript:void(0);" rel="'+data.cus_cheque_id+'" class="remove_cheqInHand" id="remove_cheqInHand_w" title="Remove cheque"><i class="fa fa-minus-square" style="font-size:24px;color:red"></i></a>'+
                                    '<div>'+
                                '</div>'; 
                            ...
                            if(cashID>0){                        
                                    $.ajax({
                                        ...
                                        success: function (res) {
                                            ...
                                            adjust_supllier_CashCredit(grnID,newCash,0); 
                                            paidGrnCash=true;
        ...
        var supplier_newid = $('#supplier-id').val();
        console.log("The supplier ID for the cheque is: prtyCheqBal" + supplier_newid);
                                //Add cheques to main cheq table
                                if ($("#tickCheque").is(':checked')){  
                                    $.ajax({
                                        type: 'post',
                                        url: "<?php echo base_url('SupplierPayment/supPaymentCheque'); ?>",
                                        data: {grnID:grnID,amount2:newPartyChq,bank:bank,chequeno:cheqno,cusChqID:cusChqID,chequedate:dateprty,date:date1},
                                        async: false,
                                        dataType:'json',  
                                        success: function(res){ 
                                            if(res>0){
                                                ...
                                                adjust_supllier_CashCredit(grnID,0,newPartyChq); 
                ...
                var ourCheqBal=0;
                    ...
                    getGrnCredit(grnID); //get the credit for entered grn  
                        if(grnCredit>=newOurChq&&grnCredit>0){ // ?????????? need changes, 
                                //Add cheques to main cheq table
                                var supplier_newid = document.getElementById('supplier-id').value;
                            console.log("The supplier ID for the cheque is if(newOurChq>0: " + supplier_newid);
                                if (($("#tickCheque").is(':checked'))&&chqAddButton==true){    // amount2= paid amount for that invoice
                                    var data = $('#chequeform').serialize()+"&grnID="+grnID+"&amount2="+newOurChq+"&cusChqID="+0+"&date="+date1+"&supplier_newid="+supplier_newid;
                                    $.ajax({
                                        type: 'post',
                                        url: "<?php echo base_url('SupplierPayment/supPaymentCheque'); ?>",
                                        data: data,
                                        ...
                                        success: function(response){ 
                                            if(response>0){
                                                ...
                                                adjust_supllier_CashCredit(grnID,0,newOurChq);                                                 
            ...
            var supCredit=0;
            var supChqeID=0;
            function saveSupPayment(){// payment by selecting all outstnding grns of supplier
                $("#save").prop('disabled', true);
                
                var sup_ID= $("#supplier-id").val(); 
                date= $("#datepicker").val();

                getSupplier_all_Credit(sup_ID);
                ...
                if(newCash>0){
                    ...
                    getSupplier_all_Credit(sup_ID); //function not created yet
                    if(supCredit>0){
                        //alert("supCredit >0 :cash: "+supCredit);
                    cashPayment(sup_ID,newCash); 
                    update_supplier_nongrn_balance(sup_ID,newCash);
                    }                                            
                }
                if(newPartyChq>0){
                    var from="pyntBySupNme";
                    ...
                    if(supCredit>0){
                       // alert("supCredit >0 : partychq "+supCredit);
                        ...
                        update_supplier_nongrn_balance(sup_ID,newPartyChq);
                    }                                             
                }
                if(newOurChq>0){
                    var from="pyntBySupNme";
                    ...
                    if(supCredit>0){
                       // alert("supCredit >0 our chq "+supCredit);
                        ...
                        update_supplier_nongrn_balance(sup_ID,newOurChq)
                    }                                             
                }
                            ...
                            $.ajax({//out standing gnrs of selected supplier
                                ...
                                success: function(data){         //                                
                                    ...
                                    for(k=0; k<data.length; k++){ 
                                        ...
                                        var creditOfGrn=parseFloat(data[k].sup_pay_credit); 
            ...
            function update_supplier_nongrn_balance(sup_id,non_Grn_balance_rest){//payment log by cash   
                              
                    if(non_Grn_balance_rest>0){ // this case run only for 1 grnid
                            $.ajax({
                                type: "Post",
                                url:"<?php echo base_url('SupplierPayment/supplier_nongrn_balance_update'); ?>",
                                data: {sup_id:sup_id,non_Grn_balance_rest:non_Grn_balance_rest},
                                async: false,
                                dataType: "json",
                                success: function (res) {                                   
                                },
            ...
            function ourChqPayment(sup_ID,new1OurChq1){
                ...
                var supChqeID=0;
                var supplier_newid = document.getElementById('supplier-id').value;
                console.log("The supplier ID for the cheque is ourChqPayment: " + supplier_newid);
           ...
           "&supplier_newid="+supplier_newid;
            ...
            function prtyChqPayment(sup_ID,new1prtychq1){ 
                new1prtychq=new1prtychq1;
                    ...
                    if ($("#tickCheque").is(':checked')){ 
                        var supplier_newid = document.getElementById('supplier-id').value;
                        console.log("The supplier ID for the cheque is new1prtychq: " + supplier_newid);//Add cheques in main cheque table  
                        $.ajax({
                            type: 'post',
                            url: "<?php echo base_url('SupplierPayment/supPaymentCheque'); ?>",
                            data: {grnID:0,amount2:new1prtychq,bank:bank,chequeno:cheqno,cusChqID:cusChqID,chequedate:dateprty,date:date,supplier_newid:supplier_newid},
                            async: false,//  cheque inserted & if  below paymnt didnt occur ?? execute only if there credit 
                            dataType:'json',  //need changes
                            success: function(supChqID){
                                if(supChqID>0){
    ...
    //alert(  paidSupCash+ " " +paidSupPrtychq+ " " +paidSupOurchq);
            ...
            function adjust_supllier_CashCredit(grn_id,cash,cheq){//update in payment table
                $.ajax({
                    type: "Post",
                    url:"<?php echo base_url('SupplierPayment/adjust_supllier_CashCredit'); ?>",
                    data: {grnID:grn_id,cash:cash,cheq:cheq}, //data: {grnID:grnID,cash:0,cheq:grnCredit},
                    async: false,
                    dataType: "json",
                    success: function (res) {  
            ...
            function getSupplier_all_Credit(sup_ID){// overall outstanding of supplier //
                $.ajax({
                    type: "Post",
                    url:"<?php echo base_url('SupplierPayment/SupplierOutstanding_by_supplier'); ?>",
                    data: {sup_ID:sup_ID},
                    async: false,
                    dataType: "json",
                    success: function (res) {  
